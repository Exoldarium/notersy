...
const storedInputValueData = async () => {
  const storedInputValuesRes = await chrome.storage.local.get('storedInputValues');
  return storedInputValuesRes;
}

const storedNoteData = async () => {
  const storedNoteRes = await chrome.storage.local.get('storedNote');
  return storedNoteRes;
}

// grab resolved data
const noteData = await storedData();
const storedInputValuesRes = await storedInputValueData();
const storedNoteRes = await storedNoteData();
