...
const getInputValues = async () => {
  const res = await chrome.storage.local.get('storedInputValues');
  const storedInputValues = res.storedInputValues;
  return storedInputValues;
};

const getStoredNoteValue = async () => {
  const res = await chrome.storage.local.get('storedNote');
  return res;
}

const selectedText = await getNoteData();
const storedInputValues = await getInputValues();
const storedNoteRes = await getStoredNoteValue();
...
selectedText.map(obj => {
  ...
  if (obj.customNote) {
    ...
    customTitleInput.value = storedInputValues?.title;
    customNoteInput.value = storedInputValues?.text;
